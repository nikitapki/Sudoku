...
public class Manager {
    ...
    private boolean checkerCombinationsFromBox(CoordinateTableSudoku coord, int value) {
        int offsetY = coord.getPosition_y() - coord.getPosition_y() % Config.SIZE_SQUARE_SUDOKU;
        int offsetX = coord.getPosition_x() - coord.getPosition_x() % Config.SIZE_SQUARE_SUDOKU;
    ...
    private boolean checkerCombinations(int indexCoordinateTableSudoku, int value) {
        ...
        return checkerCombinationsFromRow(coord.getPosition_y(), value)
    ...
    private boolean fillGrid() {
        ...
        for (int val : candidates) {
            if (checkerCombinations(index, val)) {
                gridCells.field[index].setValue(val);

                if (fillGrid()) {
                    return true;
                }

                gridCells.field[index].setValue(0);
            }
        }
        return false;
    }

    // Сделать часть клеток нефиксированными
    private void unFixedCell(int quantityRemoves) {
        ...
        for (int i = 0; i < quantityRemoves && i < massiveIndexesGrid.length; i++) {
            ...
            gridCells.field[idx].setIs_fixed(false);
        }
    }
    ...
    public boolean checkInputValueInCell(int coordinateCell, int inputValue) {
        ...
        if (gridCells.field[coordinateCell].getIs_fixed()
                || gridCells.field[coordinateCell].getValue() != inputValue) {
            result = false;
        } else {
